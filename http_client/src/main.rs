...
async fn get(pin: &str) -> Result<HashMap<String, Value>, reqwest::Error> {
    ...
    let url = format!("{}{}{}", "", pin, "");
    Ok(client.get(url).headers(headers).send().await?.json::<HashMap<String, Value>>().await?)
...
async fn post() -> Result<HashMap<String, String>, reqwest::Error> {
    ...
    Ok(client.post("https://httpbin.org/ip").headers(headers).json(&data).send().await?.json::<HashMap<String, String>>().await?)
}
